...
const CustomEvent = ({ event }) => {
  return (
    <div
      style={{ padding: "5px" }}
      className="flex flex-col justify-start gap-0 items-start w-full"
    >
      ...
      <div className="flex justify-between w-full gap-1 mt-2">
        ...
        <span className="text-sm font-light px-2 py-1  bg-white bg-opacity-80 text-black">
          {" "}
          id: {event.title.split(" - ")[4]}
        </span>
      </div>
    </div>
  );
...
const Timetable = ({ data }) => {
  ...
  const events = data?.map((item) => ({
    title: `${item.chargeHoraire?.matiere?.name} - ${item.typeSeance} - ${item.salle?.name} - ${item.chargeHoraire?.filiere?.name} - ${item.id}`,
    start: moment()
