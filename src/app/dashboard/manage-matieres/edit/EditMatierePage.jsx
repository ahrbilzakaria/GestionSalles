...
import { getMatiereById, updateMatiere } from "@/app/api/matieres";
...
export default function EditMatierePage() {
  ...
  const words = `The matiere doesn't exist.`;

  const loadMatiere = async (id) => {
    try {
      const matiereData = await getMatiereById(id);
      setMatiere(matiereData); // Set the fetched filieres to the state
    } catch (error) {
  ...
  useEffect(() => {
    ...
    loadMatiere(Number(id));
    // setFiliere({ name, capacity: Number(capacity) });
  }, [searchParams, fetched]);

  const updateMatiereTo = async () => {
    const { id, name } = matiere;
    ...
    try {
      ...
      const result = await updateMatiere(id, {
        name: name.trim(),
      });
    ...
    return (
      <div>
        <div className="flex flex-1 flex-col gap-4 p-4">
          <h1 className="font-bold text-3xl">Edit Matiere:</h1>
          <div className="min-h-[100vh] flex-1 rounded-xl md:min-h-min">
            <Card>
              <CardHeader>
                <CardTitle>Matiere</CardTitle>
                <CardDescription>
                  Make changes to your matiere here. Click save when you are
                  done.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex gap-2 flex-wrap items-center">
                <div className="max-w-[20rem]">
                  ...
                  <Input
                    ...
                    value={matiere?.name}
                    onChange={(e) =>
                      setMatiere((prev) => ({ ...prev, name: e.target.value }))
                    }
                  />
                </div>
              </CardContent>
              <CardFooter>
                <Button onClick={() => updateMatiereTo()}>Save changes</Button>
              </CardFooter>
            </Card>
          </div>
        </div>
